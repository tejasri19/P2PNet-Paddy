...
class SetCriterion_Crowd(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Compute the average number of target points across all nodes, for normalization purposes
...
# class SetCriterion_Crowd(nn.Module):
#     def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, density_alpha=2.0):
#         super().__init__()
#         self.num_classes = num_classes
#         self.matcher = matcher
#         self.weight_dict = weight_dict
...
#     def _calculate_density_weights(self, targets):
#         points_per_image = torch.tensor([len(t["labels"]) for t in targets], 
#                                     device=targets[0]["labels"].device)
#         mean_points = torch.clamp(points_per_image.float().mean(), min=1e-8)
        
#         # Normalize and clamp weights to prevent extreme values
#         density_weights = torch.clamp(points_per_image / mean_points, min=0.1, max=10.0)
#         density_weights = torch.pow(density_weights, self.density_alpha)
        
#         # Normalize weights to sum to batch size
#         density_weights = density_weights * (len(targets) / density_weights.sum())
...
#         # Calculate density-aware weights
#         density_weights = self._calculate_density_weights(targets)
        
#         # Apply per-sample weights to cross entropy loss
#         loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, 
#                                 self.empty_weight, reduction='none')
#         loss_ce = torch.nan_to_num(loss_ce * density_weights.unsqueeze(1), nan=0.0, posinf=1.0, neginf=0.0)
...
#         density_weights = self._calculate_density_weights(targets)
#         expanded_weights = torch.repeat_interleave(
#             density_weights,
#             torch.tensor([len(i[1]) for i in indices], device=density_weights.device)
#         )
        
#         loss_bbox = torch.nan_to_num(loss_bbox * expanded_weights.unsqueeze(1), 
#                                 nan=0.0, posinf=1.0, neginf=0.0)
        
#         losses = {}
#         losses['loss_points'] = loss_bbox.sum() / (num_points + 1e-8)
#         return losses
...
#                                 device=next(iter(output1.values())).device)
#         num_boxes = torch.clamp(num_points / get_world_size(), min=1).item()
