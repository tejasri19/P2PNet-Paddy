...
class HungarianMatcher_Crowd(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_point: float = 1, cost_giou: float = 1):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_point = torch.cat([v["point"] for v in targets])
        ...
        cost_point = torch.cdist(out_points,  tgt_point.cuda(), p=1)
...
#         out_points = outputs["pred_points"].flatten(0, 1).to(device)

#         tgt_ids = torch.cat([v["labels"] for v in targets])
#         tgt_points = torch.cat([v["point"] for v in targets]).to(device)
